...
<p align="center">
  <img src="screenshots/panel.png" width="300" alt="主面板：监听的代币列表与当前持仓" />
</p>
<p align="center">
  <img src="screenshots/popup.png" width="960" alt="弹卡：市值 K 线 + 群内喊单语境 + 交易卡 + 官方推特 / fomo 关注者 / GMGN 喊单 / Thesis" />
</p>
...
<p align="center">
  <img src="screenshots/dashboard-overview.png" width="480" alt="dashboard 总览" />
  <img src="screenshots/dashboard-battle.png" width="480" alt="dashboard 24h 战况" />
</p>
...
产物在 `dist/windows/`。开发调试可运行 `corepack pnpm windows:dev`；首次启动会打开群组设置，点击「创建应用并登录」，在浏览器完成飞书授权，再勾选要监听的群并保存。数据放在 `%LOCALAPPDATA%\Fomomo`（卸载不删）。悬浮窗与弹卡和 macOS 版一致：新币自动弹卡（不抢焦点，默认 6 秒后收起，点一下钉住；「设置 → 新币提醒」可改成只发系统提醒或关闭，并设置提醒条件：至少几人喊、市值范围、只看哪些链；喊单人历史胜率；首喊超过 1 小时的老币 30 分钟内又有多人喊也会提醒，行上显示 🔥 人数；弹出的卡顶部写明提醒原因）。行上显示首个喊单人的历史胜率，搜索框可搜喊单人和群名，列表上方可按来源 / 链快速筛选；`Ctrl+Alt+F` 在任何程序里显示 / 隐藏悬浮窗，设置页可看版本号、1s–1d K 线（滚轮缩放 / 拖动平移 / 双击复位，点喊单标记切换群聊语境）、官方推特与译文、GMGN 喊单（GMGN / X 两页签）、fomo 关注者与 Thesis。悬浮窗拖上 / 下 / 右边缘改尺寸；底栏可直达 24h 战况、打开内置 gmgn 窗口过 Cloudflare 验证、登录 fomo。
...
架构与各模块设计见 [docs/ARCHITECTURE.md](ARCHITECTURE.md)，术语表见 [CONTEXT.md](../CONTEXT.md)，关键决策及被否掉的替代方案见 [docs/adr/](adr/)。
